...
class ClassificationHelper():
    def __init__(self, args):
        ...
        self.model = nn.DataParallel(self.model, device_ids=[0,1], output_device=0) 
...
def main(args, input_file,fname2):
    ...
    output_file = '/workspace/2022_text_classify/patent_infer_data/inference_results/'+fname2+'_'+date_time+'_145.csv'
    #print(output_file)
...
def load_infer_data(args):
    add = 1000000
    f_path = '/workspace/2022_text_classify/patent_infer_data/TAC_ALL_202206'
    file_lst = ['CN_tac_0.csv', 'CN_tac_4.csv']
    #file_lst = ['KR_tac_test.csv']
    for fn in file_lst:
        ...
        for div_r in range(0, div):
            ...
            date_time = now.strftime("%Y_%m_%d_%H:%M")
            fn.replace('.csv', '')
            save_dir ='/workspace/2022_text_classify/patent_infer_data/inference_results/'+fn.replace('.csv', '')
